 ...
 * Replace pricing_units, {singular}, and column mappings
 */
...
try {
    ...
    error_log("pricing_units API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM pricing_units {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM pricing_units {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('pricing_units', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE pricing_units SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM pricing_units WHERE id = ?", [$id]);
