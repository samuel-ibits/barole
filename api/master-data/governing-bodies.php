 ...
 * Replace gorverning_body, {singular}, and column mappings
 */
...
try {
    ...
    error_log("gorverning_body API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM gorverning_body {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM gorverning_body {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('gorverning_body', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE gorverning_body SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM gorverning_body WHERE id = ?", [$id]);
