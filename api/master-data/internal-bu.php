 ...
 * Replace internal_bu, {singular}, and column mappings
 */
...
try {
    ...
    error_log("internal_bu API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM internal_bu {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM internal_bu {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('internal_bu', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE internal_bu SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM internal_bu WHERE id = ?", [$id]);
