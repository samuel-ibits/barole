 ...
 * Replace contract_type, {singular}, and column mappings
 */
...
try {
    ...
    error_log("contract_type API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM contract_type {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM contract_type {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('contract_type', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE contract_type SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM contract_type WHERE id = ?", [$id]);
