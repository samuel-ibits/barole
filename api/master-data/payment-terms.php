...
try {
    ...
    error_log("payment_terms API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM payment_terms {$where}", $params)->fetch()['total'];
...
function handleUpdate($db) {
    ...
    $updated = $db->query(
        "UPDATE payment_terms SET name = ?, details = ? WHERE id = ?",
        [$name, $details, $id]
    );
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM payment_terms WHERE id = ?", [$id]);
