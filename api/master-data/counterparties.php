...
try {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            break;
        case 'PUT':
        $input = parseFormDataInput(); 
        handleUpdateCounterparty($db, $input);            break;
        case 'DELETE':
            $input = parseFormDataInput(); 
            handleDeleteCounterparty($db,$input);
            break;
        default:
...
//     // Check for single record request
...
//         $counterparty = $db->query($query, [$id])->fetch();
...
//     // ---- Otherwise, run the paginated listing ----
...
//     // Format data
//     $formattedCounterparties = array_map(function($c) {
//         return [
//             'id' => $c['id'],
//             'name' => $c['name'],
//             'type' => $c['type'],
...
function handleGetCounterparties($db) {
    handleGenericGet(
        $db,
        'counterparties',
        [ 'name', 'type', 'contact_person', 'email', 'phone', 'country', 'credit_rating', 'status'],
 ...
 */
function handleUpdateCounterparty($db, $input) {
    $id = (int)($input['id'] ?? 0);
    ...
    $values[] = $id;
    $sql = "UPDATE counterparties SET " . implode(', ', $fields) . " WHERE id = ?";
    // $stmt = $db->prepare($sql);
    $success = $db->query($sql, $values);

    if (!$success) {
        sendErrorResponse('Failed to update counterparty');
        return;
    }
 ...
 */
function handleDeleteCounterparty($db,$input) {
     $id = (int)($input['id'] ?? 0);
