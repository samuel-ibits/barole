 ...
 * Replace portfolio, {singular}, and column mappings
 */
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM portfolio {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM portfolio {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('portfolio', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE portfolio SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM portfolio WHERE id = ?", [$id]);
