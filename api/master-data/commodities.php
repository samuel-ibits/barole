 ...
 * Replace commodity_name, {singular}, and column mappings
 */
...
try {
    ...
    error_log("commodity_name API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM commodity_name {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM commodity_name {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('commodity_name', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE commodity_name SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM commodity_name WHERE id = ?", [$id]);
