 ...
 * Replace discharging_port, {singular}, and column mappings
 */
...
try {
    ...
    error_log("discharging_port API error: " . $e->getMessage());
...
function handleGet($db) {
    ...
    $total = $db->query("SELECT COUNT(*) as total FROM discharging_port {$where}", $params)->fetch()['total'];

    $sql = "SELECT * FROM discharging_port {$where} ORDER BY id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
...
function handleCreate($db) {
    ...
    $newId = $db->insert('discharging_port', ['name' => $name]);
    if ($newId) {
...
function handleUpdate($db) {
    ...
    $updated = $db->query("UPDATE discharging_port SET name = ? WHERE id = ?", [$name, $id]);
...
function handleDelete($db) {
    ...
    $deleted = $db->query("DELETE FROM discharging_port WHERE id = ?", [$id]);
